export type VideoState =
    ...
    // UPLOAD
    | 'UPLOAD_PREPARING' | 'FORM_FILLING' | 'FILE_UPLOADING' | 'TIKTOK_PROCESSING' | 'FINALIZING' | 'PUBLISHED' | 'UPLOAD_FAILED'
    // SPECIAL
...
export const determineVideoStatus = (video: any, downloadJob?: any, publishJob?: any): VideoStatus => {
    ...
    // 2. Check Publish Job (Most recent activity usually)
    if (publishJob) {
        ...
        // COMPLETED
        if (status === 'completed') {
            return { state: 'PUBLISHED', message: '✅ Published successfully', color: 'green', icon: '✅' };
        }

        // RUNNING - Map detailed messages to states
        if (status === 'running') {
            if (msg.includes('Uploading')) return { state: 'FILE_UPLOADING', message: msg, color: 'blue', icon: '📤', progress: 45 }; // dynamic progress?
